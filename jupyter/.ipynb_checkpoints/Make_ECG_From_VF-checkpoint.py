...
def load_data(db_number, vf_start_time_str, data_path):
    ...
    return record, fs, vf_start_time
...
def process_data(db_number, vf_start_time_str, data_path, interval_start, interval_end, save_path):
    ...
    if record is not None:
        ...
        # ECG 데이터 추출
        data = record.p_signal[int(sample_start * fs):int(sample_end * fs) + samples_20sec]
        
        split_and_save_data(data, db_number, samples_20sec, save_path)
...
if __name__ == "__main__":
    ...
    interval_start = args.interval_start * 60
    interval_end = args.interval_end * 60

    save_path = '/smc_work/datanvme/VF/vf_before_min_{interval_start}_{interval_end}/'  # 데이터 저장 경로

    # 데이터 처리 반복
    for db_number, vf_start_time_str in databases:
        if vf_start_time_str is not None:  # VF 발생 시간이 없는 데이터는 스킵
            process_data(db_number, vf_start_time_str, data_path, interval_start, interval_end, save_path)
        else:
